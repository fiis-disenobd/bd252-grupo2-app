...
import { BackIcon, HomeIcon, ReceiveGoodsIcon } from '../components/icons/IconsAlmacen';
import { Task, Operator } from '../types';
import AssignedOperatorsModal from './AssignedOperatorsModal';

interface GoodsReceptionProps {
    onBack: () => void;
    onHome: () => void;
    onRecept: (task: Task) => void;
    tasks: Task[];
}

const GoodsReception: React.FC<GoodsReceptionProps> = ({ onBack, onHome, onRecept, tasks }) => {
    ...
    const receptionTasks = tasks.filter(
        task => task.tipo_reserva === 'Recepción' && task.status === 'En Proceso'
    );
            ...
            <div className="bg-white p-6 md:p-8 border border-gray-300 rounded-lg shadow-lg">
                <header className="flex items-start justify-between mb-6 flex-wrap gap-4">
                     <div className="flex items-center space-x-4">
                        ...
                        <h1 className="text-3xl font-bold text-gray-800">Recepción de Mercancía</h1>
                     </div>
                ...
                <div className="overflow-x-auto mt-8">
                    <table className="w-full text-sm text-left text-gray-600 border-collapse">
                        <thead className="text-xs text-white uppercase bg-[#1E3A8A]">
                            <tr>
                                {['ID RESERVA', 'TIPO_RESERVA', 'Fecha', 'Hora', 'Instalacion', 'Estado', 'Placa', 'Conductor', 'Operadores Asignados', 'Acción'].map(header => (
                            ...
                            {receptionTasks.length > 0 ? (
                                receptionTasks.map((task) => (
                                    <tr key={task.id} className="bg-white border-b hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-2 border border-gray-300">{task.placa || 'N/A'}</td>
                                        <td className="px-4 py-2 border border-gray-300">{task.conductor || 'N/A'}</td>
                                ...
                                <tr>
                                    <td colSpan={10} className="text-center py-4 px-4 border border-gray-300">No hay tareas de recepción asignadas. Asigne una desde 'Gestionar Equipo y Tareas'.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
