...
interface PickingListProps {
    onBack: () => void;
    onHome: () => void;
    onProcessPicking: (task: Task) => void;
    tasks: Task[];
}

const PickingList: React.FC<PickingListProps> = ({ onBack, onHome, onProcessPicking, tasks }) => {
    ...
    const pickingTasks = tasks.filter(
        task => task.tipo_reserva === 'Picking' && task.status === 'En Proceso'
    );
            ...
            <div className="bg-white p-6 md:p-8 border border-gray-300 rounded-lg shadow-lg">
                <header className="flex items-start justify-between mb-6 flex-wrap gap-4">
                     <div className="flex items-center space-x-4">
                        ...
                        <h1 className="text-3xl font-bold text-gray-800">Reservas de Picking Asignadas</h1>
                     </div>
                    ...
                    <table className="w-full text-sm text-left text-gray-600 border-collapse">
                        ...
                        <tbody>
                            {pickingTasks.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan={8} className="text-center py-4 px-4 border border-gray-300">No hay tareas de picking asignadas. Asigne una desde 'Gestionar Equipo y Tareas'.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
